...
def inject_signal(qcd_train_sample, sig_sample, sig_in_training_num, train_split=0.75):
    print( " inject sig ",sig_in_training_num )
    ...
    print (" sig_in_training_num ",sig_in_training_num)
    print ("sig_sample type is : ",type(sig_sample))
    if sig_in_training_num > 1. :
        print("using n events to sample")
        sig_train_sample = sig_sample.sample(n=sig_in_training_num)
    else:
        print("using fraction to sample")
        sig_train_sample = sig_sample.sample_frac(frac=sig_in_training_num)
    print ("sig_sample sampled ", sig_train_sample)
